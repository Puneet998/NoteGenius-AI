      ...
      <div className='grid grid-cols-1 lg:grid-cols-4 gap-6'>
        <AnimatePresence>
          {isSidebarOpen && <motion.div
            ...
            className="fixed lg:static top-0 left-0 z-50 lg:z-auto w-72 lg:w-auto h-full lg:h-[75vh] lg:rounded-3xl lg:col-span-1 bg-black/90 lg:bg-black/80
            backdrop-blur-xl
            ...
            overflow-y-auto"
          >
            <button onClick={() => setIsSidebarOpen(false)} className='cursor-pointer lg:hidden text-white mb-4'>
              ⬅️back
            </button>
            <div className='mb-4 space-y-1 '>
              <button onClick={() => navigate("/notes")} className='cursor-pointer w-full px-3 py-2 rounded-lg text-sm text-gray-200 bg-white/10 hover:bg-white/20 text-start'>
                ➕ New Notes
              </button>
              ...
              <h2 className='mb-4 text-lg font-bold'>
                📚 <span className='  bg-gradient-to-r from-white via-gray-300 to-white bg-clip-text text-transparent'>Your Notes ({topics.length})</span>
              </h2>
              {/* <input type='text' value={search} onChange={(e)=>setSearch(e.target.value)} placeholder='search notes' className='w-full px-3 py-2 rounded-lg text-sm text-gray-200 bg-white/10 hover:bg-white/20 text-start'/> */}
              {topics.length === 0 && (
